...
function parseBody(body) {
    if (!body || typeof body !== 'object')
        return null;
    const b = body;
    if (typeof b.action_type !== 'string' || typeof b.target_resource !== 'string')
        return null;
    return {
...
export async function gatewayRoutes(app) {
    app.post('/actions', {
        ...
        if (!agentLimit.allowed) {
            ...
            if (agentLimit.retryAfterSec)
                reply.header('Retry-After', String(agentLimit.retryAfterSec));
        ...
        if (!actionLimit.allowed) {
            ...
            if (actionLimit.retryAfterSec)
                reply.header('Retry-After', String(actionLimit.retryAfterSec));
            return reply.status(429).send({
                request_id: requestId,
                timestamp: new Date().toISOString(),
                version: 'v1',
        ...
        try {
            ...
            if (capRes.rows[0])
                agentCapabilities = capRes.rows[0].capabilities ?? [];
        }
        catch {
            // ignore
        }
        ...
        const downstream = await proxyToDownstream(body.action_type, body.target_resource, body.parameters ?? {}, body.context);
        await appendEvent({
            event_type: 'outcome',
    ...
    app.get('/actions/:request_id/status', {
        preHandler: [app.requireAgentAuth()],
    }, async (request, reply) => {
        const agentId = request.agentId;
        ...
        const cached = await cacheGet(gatewayRequestKey(request_id));
        if (!cached?.trace_id) {
        ...
        let decision;
        let approval_id = cached.approval_id;
        let denial_reason;
