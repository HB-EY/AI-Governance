...
export async function dashboardRoutes(app) {
    ...
    app.get('/metrics', {
        ...
        const [agentsRes, tracesRes, approvalsRes, recentRes, violationsRes] = await Promise.all([
            pool.query(`SELECT status, COUNT(*)::int AS c FROM agents GROUP BY status`),
            pool.query(`SELECT status, COUNT(*)::int AS c FROM traces WHERE request_timestamp >= $1 GROUP BY status`, [since24h]),
            pool.query(`SELECT COUNT(*)::int AS c FROM approval_requests WHERE status = 'pending'`),
            pool.query(`SELECT t.id, t.agent_id, t.action_type, t.status, t.request_timestamp, a.name AS agent_name
         FROM traces t LEFT JOIN agents a ON a.id = t.agent_id
         ORDER BY t.request_timestamp DESC LIMIT 20`),
            pool.query(`SELECT t.id, t.agent_id, t.action_type, t.status, a.name AS agent_name
         FROM traces t LEFT JOIN agents a ON a.id = t.agent_id
         WHERE t.status = 'denied' ORDER BY t.request_timestamp DESC LIMIT 5`),
        ]);
        const agentsByStatus = { active: 0, disabled: 0, suspended: 0 };
        for (const row of agentsRes.rows) {
            agentsByStatus[row.status] = row.c;
        }
        const actions24h = { success: 0, denied: 0, failed: 0, pending: 0 };
        for (const row of tracesRes.rows) {
            actions24h[row.status] = row.c;
        }
        const pending_approvals = approvalsRes.rows[0]?.c ?? 0;
        const recent_activity = recentRes.rows.map((r) => ({
            id: r.id,
            agent_id: r.agent_id,
        ...
        const recent_violations = violationsRes.rows.map((r) => ({
            id: r.id,
            agent_id: r.agent_id,
            agent_name: r.agent_name,
            action_type: r.action_type,
