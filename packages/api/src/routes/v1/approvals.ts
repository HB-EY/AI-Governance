...
export async function approvalRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get<{ Params: { approval_id: string } }>('/:approval_id', async (request, reply) => {
    ...
    let evidence_summary: Record<string, unknown> = {};
    try {
      const key = evidenceKey(approval.trace_id);
      const raw = await downloadEvidence(key);
      if (raw) evidence_summary = JSON.parse(raw) as Record<string, unknown>;
    } catch {
      // ignore
    }
  ...
  app.get<{ Querystring: { status?: string; approver_id?: string; approver_role?: string; page?: string; page_size?: string } }>('/', async (request, reply) => {
    const q = request.query as { status?: string; approver_id?: string; approver_role?: string; page?: string; page_size?: string };
    const page = Math.max(1, parseInt(q.page ?? '1', 10));
    const pageSize = Math.min(50, Math.max(1, parseInt(q.page_size ?? '20', 10)));
    const { items, total } = await listApprovals(
      { status: q.status ?? 'pending', approver_id: q.approver_id, approver_role: q.approver_role },
      { page, pageSize }
    );
    return reply.send({
      request_id: request.id,
