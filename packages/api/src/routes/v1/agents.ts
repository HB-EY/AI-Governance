...
const validateRegister = createValidator<RegisterAgentRequest>(registerAgentSchema);

/** Owner format: allow alphanumeric, email, or UUID (identity provider patterns). */
function isValidOwner(ownerId: string): boolean {
  if (!ownerId || ownerId.length > 255) return false;
...
export async function agentRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post<{
    ...
    const result = validateRegister(request.body as RegisterAgentRequest);
    if (!result.success) {
    ...
    const validSet = new Set<string>(VALID_CAPABILITY_TYPES);
    ...
    try {
      const userId = (request.headers['x-user-id'] as string) ?? body.owner_id;
      await createAgent(
        {
          id: agentId,
          name: body.name,
    ...
    const response: RegisterAgentResponse = { agent_id: agentId, api_key: rawKey };
  ...
  app.get<{
    ...
    try {
      const actionsRes = await pool.query(
        `SELECT COUNT(*)::int AS c FROM traces WHERE agent_id = $1 AND request_timestamp >= NOW() - INTERVAL '30 days'`,
        [agent_id]
      );
      actionsCount = (actionsRes.rows[0] as { c: number })?.c ?? 0;
      const violRes = await pool.query(
        `SELECT COUNT(*)::int AS c FROM traces WHERE agent_id = $1 AND status = 'denied' AND request_timestamp >= NOW() - INTERVAL '30 days'`,
        [agent_id]
      );
      violationsCount = (violRes.rows[0] as { c: number })?.c ?? 0;
    } catch {
      // ignore
    }
  ...
  app.get<{
    Querystring: { page?: string; page_size?: string; status?: string; owner?: string; name?: string };
  }>('/', async (request, reply) => {
    const q = request.query as { page?: string; page_size?: string; status?: string; owner?: string; name?: string };
    ...
    if (q.status) filters.status = q.status as AgentListFilters['status'];
    if (q.owner) filters.owner_id = q.owner;
  ...
  app.patch<{
    Params: { agent_id: string };
    Body: { description?: string; owner_id?: string; owner_email?: string | null; capabilities?: string[] };
  }>('/:agent_id', {
    preHandler: [app.requireUserAuth()],
  }, async (request, reply) => {
    const { agent_id } = request.params as { agent_id: string };
    const body = (request.body as { description?: string; owner_id?: string; owner_email?: string | null; capabilities?: string[] }) ?? {};
    const agent = await getAgentById(agent_id);
    if (!agent) {
      return reply.status(404).send({
        request_id: request.id,
    ...
    if (!updated) return reply.status(404).send({ request_id: request.id, timestamp: new Date().toISOString(), version: 'v1', error: { code: 'not_found', message: 'Agent not found' } });
