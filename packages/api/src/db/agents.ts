...
const AGENT_COLS =
  'id, name, description, owner_id, owner_email, status, current_version_id, api_key_hash, created_at, updated_at, created_by, updated_by, last_active_at';
...
function mapRow(row: Record<string, unknown>): Agent {
  return {
    ...
    last_active_at: row.last_active_at ? (row.last_active_at as Date).toISOString() : null,
  };
}

export async function getAgentById(id: string, client?: PoolClient): Promise<Agent | null> {
...
export async function listAgents(
  options: ListOptions & { filters?: AgentListFilters },
  client?: PoolClient
  ...
  const countRes = await db.query(
    'SELECT COUNT(*)::int AS total FROM agents ' + whereSql,
    params
  );
  ...
  const sql =
    'SELECT ' + AGENT_COLS + ' FROM agents ' + whereSql + order.sql + page.sql;
...
export async function createAgent(
  ...
  return withTransaction(async (client) => {
    await client.query(
      `INSERT INTO agents (id, name, description, owner_id, owner_email, api_key_hash, created_by, status)
    ...
    await client.query(
      `INSERT INTO agent_versions (id, agent_id, version_number, status, purpose, risk_tier, capabilities)
       VALUES ($1, $2, 1, 'active', 'Initial registration', 'low', $3)`,
      [versionId, data.id, capabilities]
    );
    await client.query(
      `UPDATE agents SET current_version_id = $1, updated_at = NOW() WHERE id = $2`,
      [versionId, data.id]
    );
    const out = await getAgentById(data.id, client);
    if (!out) throw new Error('Agent not found after create');
    return out;
  });
}
...
export async function updateAgentMetadata(
  id: string,
  data: { description?: string; owner_id?: string; owner_email?: string | null; capabilities?: string[] },
  client?: PoolClient
): Promise<Agent | null> {
  const db = client ?? getPool();
  ...
  if (updates.length === 0 && !data.capabilities?.length) return getAgentById(id, client);
  ...
  if (data.capabilities && data.capabilities.length > 0) {
    ...
    await db.query(
      `INSERT INTO agent_versions (id, agent_id, version_number, status, purpose, risk_tier, capabilities)
       VALUES ($1, $2, $3, 'active', 'Capability update', 'low', $4)`,
      [versionId, id, nextVersion, data.capabilities]
    );
    await db.query(
      `UPDATE agents SET current_version_id = $1, updated_at = NOW() WHERE id = $2`,
      [versionId, id]
...
async function getNextVersionNumber(agentId: string, client?: PoolClient): Promise<number> {
  const db = client ?? getPool();
  const res = await db.query(
...
export async function findAgentByName(name: string, client?: PoolClient): Promise<Agent | null> {
  const db = client ?? getPool();
