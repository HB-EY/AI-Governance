...
export async function runApprovalTimeout(): Promise<number> {
  const pool = getPool();
  if (!pool) return 0;
  const res = await pool.query(
    `SELECT id, trace_id, agent_id FROM approval_requests WHERE status = 'pending' AND expires_at < NOW()`
  );
  const rows = res.rows as { id: string; trace_id: string; agent_id: string }[];
  let count = 0;
  for (const row of rows) {
    ...
    await pool.query(
      `UPDATE traces SET status = 'denied', completed_at = NOW() WHERE id = $1`,
      [row.trace_id]
    );
