...
export async function runEvidenceExport(job: Job): Promise<void> {
  ...
  const format = (payload?.format === 'csv' ? 'csv' : 'json') as 'json' | 'csv';
  ...
  if (format === 'csv') {
    ...
    const out = rows.map((r) => {
      ...
      if (obj.request_payload && typeof obj.request_payload === 'object') {
        ...
        try {
          obj.request_payload = JSON.parse(obj.request_payload as string);
        } catch {
          // leave as string
        }
      }
  ...
  await pool.query(
    `UPDATE jobs SET payload = payload || $1::jsonb WHERE id = $2`,
    [JSON.stringify({ result_key: resultKey }), job.id]
  );
}
