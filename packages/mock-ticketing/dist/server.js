...
app.patch('/tickets/:id', (req, res) => {
    ...
    if (assigned_to !== undefined)
        ticket.assigned_to = assigned_to == null ? null : String(assigned_to);
    res.json(ticket);
});
/** Gateway downstream callback: POST with action_type, target_resource, parameters */
app.post('/v1/action', (req, res) => {
    const { action_type, target_resource, parameters = {} } = (req.body ?? {});
    ...
    if (action_type === 'ticket-read') {
        ...
        if (!ticket)
            return res.status(404).json({ error: 'Ticket not found', id });
    ...
    if (action_type === 'ticket-update') {
        ...
        if (!ticket)
            return res.status(404).json({ error: 'Ticket not found', id });
        const { priority, category, assigned_to } = parameters;
        if (priority)
            ticket.priority = priority;
        if (category)
            ticket.category = category;
